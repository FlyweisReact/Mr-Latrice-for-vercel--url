...
const ClientReviews = () => {
  const [activeTab, setActiveTab] = useState("What Clients Love");
  ...
  const growthTags = [
    "Exceeded The Indicated Service Time",
    "Noisy & Disturbed Environment",
    "Not Attentive To Customers Needs",
    "Dirty Tools and Floor",
    "Bad Product Used",
    "Poor Hygiene",
    "Overpriced for the Quality",
    "Unprofessional Staff",
    "Appointment Was Ignored",
    "Wrong Hair Color",
    "Long Wait Despite Appointment",
    "Unfriendly Staff",
    "Poor Communication",
    "Inconsistent Service Quality",
    "Overcharged Without Clear Pricing",
    "Constantly Stopped Service",
  ];
    ...
    <div className="w-full">
      ...
      <div className="border border-[#2F2F2F33] bg-[#FFFFFF] rounded-[10px] px-5 py-4 mb-4">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-[800] sm:text-[20px] text-[15px] text-charcoal font-sansation">
            Omar Vaccaro’s Reviews
          </h2>
          <div className="flex items-center mt-1">
            <IoIosStar className="text-[#2F2F2F] text-xl" />
            <span className="font-[800] sm:text-[20px] text-[18px] text-charcoal font-outfit">
              5.0
            </span>
            <span className="font-[600] sm:text-[15px] text-[12px] text-[#2F2F2F80] font-outfit ml-1">
              (417)
            </span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={index} className="flex items-center gap-2">
              ...
              <span className="font-[600] sm:text-[17px] text-[15px] text-charcoal font-sansation">
                {percent}
              </span>
            </div>
          ))}
      ...
      <div className="mb-4">
        <div className="flex items-center gap-2 mb-3">
          {/* What Clients Love */}
          <div
            className={`px-3 py-1.5 sm:text-[20px] text-[18px] font-[600] font-rasa rounded-[10px] cursor-pointer transition-all duration-300 ${
          ...
          <div
            className={`px-3 py-1.5 sm:text-[20px] text-[18px] font-[600] font-rasa rounded-[10px] cursor-pointer transition-all duration-300 ${
              activeTab === "Areas of Growth"
                ? "bg-[#EB0303] text-[#FAF9F6]"
                : "text-[#2F2F2F]"
            }`}
        ...
        {activeTab === "What Clients Love" ? (
            ...
            {positiveTags.map((tag, index) => (
              <div className="flex items-center" key={index}>
                ...
                {index !== positiveTags.length - 1 && (
                  <span className="sm:text-[16px] text-[14px] font-[600] font-rasa ml-1">
                    ,
                  </span>
                )}
              </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {growthTags.map((tag, index) => (
              <div
                key={index}
                className="bg-[#FFEBEB] text-[#EB0303] px-2 py-1 sm:text-[16px] text-[14px] font-[600] font-rasa rounded-[5px]"
              >
                {tag},
              </div>
            ))}
            ...
            <div className="flex gap-3 w-full">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="font-[700] sm:text-[14px] text-[12px] text-[#121111] font-outfit">
                    {item.salonName}
                  </span>
                ...
                {/* Review Text */}
                <p className="font-[400] sm:text-[14px] text-[12px] text-[#757575] font-outfit mt-5">
                  {item.review}
                </p>
                <div className="flex items-center justify-end mt-3 gap-2">
                  <button className="bg-[#FFCC4E] px-2.5 py-0.5 rounded-[6px] font-[500] sm:text-[14px] text-[12px] text-[#000000]">
                    Reply
                  </button>
